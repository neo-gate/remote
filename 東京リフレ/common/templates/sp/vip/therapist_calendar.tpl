...
<div style="margin:10px auto 0px auto;width:320px;">
	<hr />
</div>

<div style="margin:10px auto 30px auto;width:320px;">

	<div style="padding:0px 20px 10px 20px;">
		<div style="font-weight:bold;font-size:12px;">
			〇セラピスト出勤カレンダー&予約フォーム
		</div>
		<div style="font-size:12px;padding:10px 0px 0px 0px;float:right;">
	...
	<div style="margin:0px 0px 10px 20px;">
		<div>縦スクロールでセラピスト移動</div>
		<div style="padding:5px 0px 0px 0px;">横スクロールで時間移動</div>
		<div style="padding:5px 0px 0px 0px;">○の時間帯のご予約が可能です</div>
		<div style="padding:5px 0px 0px 0px;">○をクリックするとご予約フォームに移動します</div>
	</div>
			...
			<div id="calendar_display_content">
				...
				<div class="gyou_new">
					...
					{{section name=cnt start=1 loop=24}}
					    ...
					    {{if $gyou_type == 1}}
					    	...
					    	<div class="maru1" style="background:#ccc;">
					    	{{else}}
							<div class="maru1">
							{{/if}}
						{{else}}
							{{if $params.free_therapist_state[$z] == 0}}
					    	<div class="maru2" style="background:#ccc;">
					    	{{else}}
							<div class="maru2">
							{{/if}}
						{{/if}}
				...
				<div class="gyou_new">
					...
					{{section name=cnt2 start=1 loop=24}}
						...
						{{math equation=a-b a=$params.attendance_data[cnt].end_time b=0 assign=end_time}}
						...
						{{if ($j >= $start_time) && ($j <= $end_time)}}
							...
							<div>
								...
								{{if $reservation_flag == true}}
									...
									{{if $gyou_type == 1}}
										<div id="{{$attendance_id}}_{{$j}}" class="batsu1" style="background:#ccc;">×</div>
									{{else}}
										<div id="{{$attendance_id}}_{{$j}}" class="batsu2" style="background:#ccc;">×</div>
									{{/if}}

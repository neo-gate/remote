...
$(function() {
	...
	$("#reserv_day").change(function(){
		...
		$.ajax({
			type:'post',
			url:url_root_ssl+'ajax/vip/free_therapist_3.php',
			data:{
...
//function action_history_clip_common(id,selected_num,type,area){
function action_history_clip_common(id,selected_num,type,area,loginCustomId){	//update by aida at 20180320
	var url_root = document.location.protocol + "//"+location.hostname+"/";
	var url_root_ssl = url_root;
	...
	$.ajax({
		...
		success:function(data){
			var url = url_root_ssl+"ssl/"+area+"/vip/history.php?selected_num="+selected_num;
			location.href = url;
			exit;
		}
	});
